// Preview team page — Variant D (smart cells with delta)
import { notFound } from "next/navigation";
import Link from "next/link";
import TeamGameLogTableD from "@/components/TeamGameLogTableD";
...
export default async function TeamPreviewD({ params, searchParams }: Props) {
  ...
  return (
    <div>
      ...
      {seasons.length > 1 && (
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          {seasons.map((s) => (
            <Link
              key={s}
              href={`/preview/team-d/${abbr}?season=${s}`}
      ...
      <div className="nba-eyebrow mb-2">Game log — smart cells variant</div>
      <p className="text-[11px] text-(--nba-muted) mb-3 max-w-2xl">
        Each cell shows team value, opponent value (faded), and the delta colored green/red. One row per game, denser cells.
      </p>
      <TeamGameLogTableD rows={log} teamAbbr={team.abbreviation} />

      <div className="mt-6 flex justify-between text-[11px] text-(--nba-muted)">
        <Link href={`/preview/team-c/${abbr}?season=${season}`}>
          ← See variant C (stacked rows)
        </Link>
